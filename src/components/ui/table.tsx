...
const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        "h-9 whitespace-nowrap px-3 py-2 text-left align-middle text-[0.65rem] font-semibold uppercase tracking-[0.07em] text-muted-foreground bg-muted/45 [&:has([role=checkbox])]:pr-0",
        className,
      )}
      {...props}
    />
...
const TableCell = React.forwardRef<HTMLTableCellElement, React.TdHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <td
      ref={ref}
      className={cn(
        "px-3 py-2 align-middle text-[0.8125rem] leading-snug [&:has([role=checkbox])]:pr-0",
        className,
      )}
      {...props}
    />
